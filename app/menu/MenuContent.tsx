...
export default function MenuContent({ initialProducts }: MenuContentProps) {
  ...
  return (
    <>
      {/* ══════════════════════════════════════════════════════
          HEADER — una sola fila + tab bar debajo
      ══════════════════════════════════════════════════════ */}
        ...
        <div className="max-w-7xl mx-auto px-4 h-[60px] flex items-center gap-3">
          ...
          <Link href="/" className="flex items-center gap-2.5 flex-shrink-0">
            ...
            <div className="leading-none hidden sm:block">
              <p className="text-stone-400 text-[10px] font-medium mb-0.5">Carta Digital</p>
              <p className="text-white font-black text-sm leading-none">
                <span className="text-amber-400">¡Qué</span> Bravazo!
              </p>
            </div>
          </Link>
          ...
          <div className="relative flex-1 mx-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 bg-stone-800 border border-stone-700 rounded-xl text-sm text-white placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 focus:bg-stone-700/80 transition-all"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <button
            ...
            className="relative flex-shrink-0 flex items-center justify-center bg-amber-500 hover:bg-amber-400 active:scale-95 text-white rounded-xl transition-all shadow-lg"
      ...
      <main className="max-w-7xl mx-auto px-4 py-5 pb-32">
        ...
        {search && (
          <div className="mb-5 flex items-center gap-2">
            ...
            <button onClick={() => setSearch('')} className="ml-auto text-xs text-amber-600 hover:text-amber-700 font-semibold flex items-center gap-1">
          ...
          {Array.from(groupedProducts.entries()).map(([category, products]) => (
            <section key={category}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs text-stone-400 bg-stone-100 px-2 py-0.5 rounded-full font-semibold">{products.length}</span>
        ...
        {!search && (
          <div className="mt-12 text-center">
            <Link
              href="/"
              className="inline-flex items-center gap-2 bg-stone-800 hover:bg-stone-900 text-white px-6 py-3 rounded-full font-semibold text-sm transition-all shadow-md"
            >
